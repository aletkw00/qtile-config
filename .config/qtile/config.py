...
keys = [
    ...
    Key(
        ...
        lazy.spawn(files.resolve_file_path("${scripts}/notify-volume.sh up")),
        desc="Increase volume",
    ),
    Key(
        [],
        "XF86AudioLowerVolume",
        lazy.spawn(files.resolve_file_path("${scripts}/notify-volume.sh down")),
        desc="Decrease volume",
    ),
    Key(
        [],
        "XF86AudioMute",
        lazy.spawn(files.resolve_file_path("${scripts}/notify-volume.sh mute")),
        desc="Mute audio",
    ),
    ...
    Key(
        [],
        "XF86MonBrightnessUp",
        lazy.spawn(files.resolve_file_path("${scripts}/notify-brightnessctl.sh up")),
        desc="Increase brightness",
    ),
    Key(
        [],
        "XF86MonBrightnessDown",
        lazy.spawn(files.resolve_file_path("${scripts}/notify-brightnessctl.sh down")),
    ...
    Key(
        ...
        "d",
        lazy.spawn(
            files.resolve_file_path("${rofi_home}/launchers/type-6/launcher.sh")
        ),
        desc="Run launcher",
    ),
    Key(
        [mod],
        "p",
        lazy.spawn(files.resolve_file_path("${scripts}/set-random-wallpaper.sh")),
...
def init_colors():
    ...
    return [colordict[f"color{i}"] for i in range(16)]
...
decor_rounded_group = {
    "decorations": [
        RectDecoration(
            colour=colors[2], radius=13, filled=True, padding_y=0, group=True
        )
    ]
}
...
def init_widgets_list():
    widgets_list = [
        ...
        widget.TextBox(text="|", foreground=colors[9], padding=2, fontsize=fontsize),
        widget.LaunchBar(
            progs=[
        ...
        widget.TextBox(text="|", foreground=colors[9], padding=2, fontsize=fontsize),
        widget.WindowName(
            foreground=colors[6], padding=8, fontsize=fontsize, max_chars=40
        ),
        ...
        widget.CPU(
            ...
            fontsize=fontsize,
            mouse_callbacks={
                "Button1": lambda: qtile.cmd_spawn(f"{terminal} -e bash -ic htop")
            },
        ...
        widget.Image(
            ...
            mouse_callbacks={
                "Button1": lambda: qtile.cmd_spawn(
                    files.resolve_file_path("${rofi_home}/applets/bin/powermenu.sh")
                )
            },
            **decor_rounded,
        ),
    ...
    with open("/home/alessio/.autostart.log", "w") as f:
        ...
        try:
            home = os.path.expanduser(
                files.resolve_file_path("${scripts}/autostart.sh")
            )
            subprocess.Popen([home], stdout=f, stderr=f)
            subprocess.Popen(
                ["picom", "--config", files.resolve_file_path("${picom}/picom.conf")],
                stdout=f,
                stderr=f,
            )
